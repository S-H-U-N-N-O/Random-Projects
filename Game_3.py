...
median_r = Entity(model='cube', collider='box', position=(25, 2, 0), scale=(5, 10, 1000000), color=color.black)
median_l = Entity(model='cube', collider='box', position=(-25, 2, 0), scale=(5, 10, 1000000), color=color.black)
...
def update():
    ...
    if held_keys["a"]:
        ...
        player.rotation_z = player.rotation_z-time.dt*100
    if held_keys["w"]:
        player.z = player.z + player.z + time.dt*1000
